                    ...
                    <form class="form-horizontal form-label-left input_mask" method="post"
                        ...
                        <div class="col-xs-12 form-group">
                            ...
                            <select name="faculty_id" id="faculty" class="form-control @error('faculty') bad @enderror">
                                @foreach($faculties as $faculty)
                                    <option
                                        value="{{ $faculty->id }}" {{ $student->department->faculty_id == $faculty->id ? 'selected' : '' }}>{{ $faculty->name }}</option>
                                @endforeach
                            </select>
                            @error('faculty')
                        ...
                        <div class="col-xs-12 form-group">
                            ...
                            <select name="department_id" id="department"
                                ...
                                @foreach(\App\Faculty::where('id', $student->department->faculty_id)->first()->departments as $department)
                                    <option
                                        value="{{ $department->id }}" {{ $student->department_id == $department->id ? 'selected' : '' }}>{{ $department->name }}</option>
                                @endforeach
                            </select>
                            @error('department')
                            <span class="text-danger">{{ $message }}</span>
                        ...
                        <div class="col-xs-12 form-group">
                            <label for="academic_advisor_id">Academic Advisor</label>
                            <select name="academic_advisor_id" id="academic_advisor_id"
                                    class="form-control @error('academic_advisor_id') bad @enderror">
                                @foreach(\App\Department::where('id', $student->department_id)->first()->academicAdvisors as $academicAdvisor)
                                    <option
                                        value="{{ $academicAdvisor->user->id }}" {{ $student->academic_advisor_id == $academicAdvisor->user->id ? 'selected' : '' }}>{{ $academicAdvisor->user->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-xs-12 form-group @error('name') bad @enderror">
                            ...
                            <input type="text" class="form-control" id="name" name="name"
                                   placeholder="Name" value="{{ $student->user->name }}">
                        ...
                        <div class="col-xs-12 form-group @error('mobile') bad @enderror">
                            ...
                            <input type="text" class="form-control" id="mobile" name="mobile"
                                   placeholder="Mobile" value="{{ $student->user->mobile }}">
                        ...
                        <div class="col-xs-12 form-group">
                            ...
                            <select name="gender" id="gender" class="form-control @error('gender') bad @enderror">
                                <option value="male" {{ $student->user->gender == 'male' ? 'selected' : '' }}>Male
                                </option>
                        ...
                        <div class="col-xs-12 form-group">
                            ...
                            <select name="nationality" id="nationality"
                                ...
                                <option
                                    value="egyptian" {{ $student->user->nationality == 'egyptian' ? 'selected' : '' }}>
                                    Egyptian
                                </option>
                                <option value="other" {{ $student->user->nationality == 'other' ? 'selected' : '' }}>
                                    Other...
                                </option>
                            </select>
                            @error('nationality')
                        ...
                        <div class="col-xs-12 xdisplay_inputx form-group">
                            ...
                            <input type="date" class="form-control @error('birth_date') bad @enderror" name="birth_date"
                                   id="birth_date"
                                   aria-describedby="birth_date" value="{{ $student->user->birth_date }}">
                            @error('birth_date')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="col-xs-12 form-group">
                            ...
                            <select name="religion" id="religion" class="form-control @error('religion') bad @enderror">
                                <option value="muslim" {{ $student->user->religion == 'muslim' ? 'selected' : '' }}>
                                    Muslim
                                </option>
                                <option
                                    value="christian" {{ $student->user->religion == 'christian' ? 'selected' : '' }}>
                                    Christian
                                </option>
                                <option value="other" {{ $student->user->religion == 'other' ? 'selected' : '' }}>
                                    Other...
                                </option>
                            </select>
                            @error('religion')
